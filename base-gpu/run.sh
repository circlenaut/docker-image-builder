...
select opt in "${options[@]}"; do
    case $opt in
        ...
        "Ubuntu 20.04 with Cuda 11.0 and cuDNN8")
            echo "Running image: $opt"
            if [ "$1" == "-i" ]; then
                sudo docker container run --name focal_cuda_11-0_cudnn8 -it --rm --gpus all registry.dev.proneer.co/cuda:11.0-cudnn8-devel-ubuntu20.04 /bin/bash
            else
	            sudo docker container run --name focal_cuda_11-0_cudnn8 --rm --gpus all registry.dev.proneer.co/cuda:11.0-cudnn8-devel-ubuntu20.04
            fi
            break
            ;;
        "Ubuntu 20.04 with Cuda 11.1 and cuDNN8")
            echo "Running image: $opt"
            if [ "$1" == "-i" ]; then
	            sudo docker container run --name focal_cuda_11-1_cudnn8 -it --rm --gpus all registry.dev.proneer.co/cuda:11.1-cudnn8-devel-ubuntu20.04
            else
                sudo docker container run --name focal_cuda_11-1_cudnn8 --rm --gpus all registry.dev.proneer.co/cuda:11.1-cudnn8-devel-ubuntu20.04
            fi
	        break
            ;;
        "Quit")
